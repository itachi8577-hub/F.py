...
# =====================================================================
# ⚙️ CONFIG - YAHI SE CHANGE KARO
# =====================================================================
API_ID = 24923714
API_HASH = '040929ee690bdb53b36484e017310358'
BOT_TOKEN = '8871635920:AAEOWScAkt3sPF5DDzJarffx3jCffrXRb8w'  # @BotFather se lo
OWNER_ID = 7862394625  # Teri ID

SUDO_USERS = [OWNER_ID]
bot_status = "online"
raid_active = {}
raid_speed = {"r": 1.5, "rr": 0.3, "rrr": 0.08}  # 🔥 0.08s = ~12 msg/sec
GROUPS_CACHE = []
muted_users = {}
USER_STATES = {}
BOT_USERNAME = "@YOUR_FATHERR_BOT"
...
async def command_handler(event):
    ...
    # ===== .speed =====
    if cmd == "speed":
        await event.reply(f"🚀 **RAID SPEEDS:**\n`.r` = `{raid_speed['r']}s`\n`.rr` = `{raid_speed['rr']}s`\n`.rrr` = `{raid_speed['rrr']}s` (~12 msg/sec) 🔥")
        return
    ...
    if cmd in ["r", "rr", "rrr"]:
        ...
        # 🔥 SPEED FIXED: 0.08s for rrr = 12+ msg/sec
        speed_map = {"r": 1.5, "rr": 0.3, "rrr": 0.08}
        speed = speed_map.get(cmd, 1.5)
        
        mention = get_mention(target_user or await bot.get_entity(target_id))
        
        await event.reply(f"╔══════════════════════╗\n  ⚔️ **RAID STARTED** ⚔️\n╚══════════════════════╝\n\n🎯 {mention}\n⚡ `{speed}s` (~{round(1/speed)} msg/sec)\n📜 `{len(raid_lines)}` lines\n\n🔥 Raid chal raha hai...")
    ...
    if cmd == "spam":
        ...
        for _ in range(count):
            ...
            await asyncio.sleep(0.08)  # 🔥 0.08s = ultra fast spam
        await event.reply(f"✅ Done!")
        return
